...
local function StartAutoFarm()
    ...
    if not AutoFarm.CurrentMob then
        return
    end
...
local InfoLabel4 = AutoFarmTab:AddLabel("Fast farming - moves immediately to next mob")
...
task.spawn(function()
    ...
    if WorldDropdown then
        local mobs = GetMobsInWorld("Dragon Town")
        MobDropdown:Set(mobs)
    end
end)
